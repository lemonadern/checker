    ...
    <table className="min-w-full bg-white border border-[#d0d0d0]">
      <thead>
        <tr className="bg-[#fafafa]">
          <th className="border-b border-[#d0d0d0] px-4 py-2 text-left">授業科目</th>
          <th className="border-b border-[#d0d0d0] px-4 py-2 text-left">学科</th>
          <th className="border-b border-[#d0d0d0] px-4 py-2 text-left">単位数</th>
          <th className="border-b border-[#d0d0d0] px-4 py-2 text-left">学年</th>
          <th className="border-b border-[#d0d0d0] px-4 py-2 text-left">履修状態</th>
        </tr>
      </thead>
      <tbody>
        {syllabusItems.map((item: SyllabusItem, index: number) => (
          <tr key={index} className={`${getStatusBgColor(courseStatuses[item.科目番号])}`}>
            <td className="border-b border-[#d0d0d0] px-4 py-2">{item.授業科目}</td>
            <td className="border-b border-[#d0d0d0] px-4 py-2">{item.学科}</td>
            <td className="border-b border-[#d0d0d0] px-4 py-2">{item.単位数}</td>
            <td className="border-b border-[#d0d0d0] px-4 py-2">{item.学年}</td>
            <td className="border-b border-[#d0d0d0] px-4 py-2">
              <select 
                className={`w-full p-2 border border-[#d0d0d0] rounded ${getSelectBgColor(courseStatuses[item.科目番号])} focus:ring-2 focus:ring-[#000] focus:outline-none`}
                value={courseStatuses[item.科目番号] || '未履修'}
                onChange={(e) => onStatusChange(item.科目番号, e.target.value as CourseStatus)}
              >
