...
export const checkInformationScienceRequiredCourses: RequirementCheckFn = (
  ...
  const allCoursesExist = INFORMATION_SCIENCE_REQUIRED_COURSES.every((courseName) => 
    requiredCourses.some((item) => item.授業科目 === courseName)
  );

  // シラバスに含まれていない科目リスト
  const missingCourses = INFORMATION_SCIENCE_REQUIRED_COURSES.filter((courseName) => 
    !requiredCourses.some((item) => item.授業科目 === courseName)
  );
  ...
  let message = "";
  if (!allCoursesExist) {
    message = `シラバスに含まれていない必修科目があります: ${missingCourses.join(", ")}`;
  } else if (!satisfied) {
    message = `必修科目の単位修得が不足しています（現在: ${completedCourses.length}科目、必要: ${totalRequiredCourses}科目）`;
  } else {
