...
export const checkJabeeComputerArchitectureRequirement: RequirementCheckFn = (
  ...
  if (computerArchitectureCourses.length === 0) {
    return {
      name:
        "JABEE専門科目群：1. コンピュータシステム系科目について、コンピュータアーキテクチャの履修",
      satisfied: false,
  ...
  return {
    ...
    details: {
      ...
      incompleteItems: computerArchitectureCourses.filter(
        (course) =>
          !completedCourses.some((c) => c.科目番号 === course.科目番号),
      ),
    },
  };
};
...
export const checkJabeeComputerSystemCoursesRequirement: RequirementCheckFn = (
  ...
  return {
    name:
      "JABEE専門科目群：2. コンピュータシステム系科目について、5科目以上の単位修得",
    satisfied,
    message: satisfied
...
export const checkJabeeSystemProgrammingRequirement: RequirementCheckFn = (
  ...
  if (!allCoursesExist) {
    ...
    return {
      name:
        "JABEE専門科目群：3. システムプログラム系科目についてアルゴリズムとデータ構造・プログラミング言語論・ソフトウェア設計をすべて履修し、これらの科目から二科目以上の単位修得",
      satisfied: false,
      message: `システムプログラム系の必要科目が不足しています: ${
        missingCourses.join(", ")
      }`,
      details: {
  ...
  const isAllEnrolled = SYSTEM_PROGRAMMING_COURSES.every((courseName) =>
    systemProgrammingCourses.some((item) =>
      ...
      ["単位取得済み", "履修予定", "履修中"].includes(
        courseStatuses[item.科目番号] || "",
      )
    )
  );
  ...
  const isTwoOrMoreCompleted =
    completedCourses.length >= requiredCompletedCount;
  ...
  if (satisfied) {
    message =
      `システムプログラム系の全科目を履修し、${completedCourses.length}科目の単位を修得済みです（要件: 全科目履修かつ${requiredCompletedCount}科目以上の単位修得）`;
    ...
    message = `システムプログラム系の以下の科目が未履修です: ${
      notEnrolledCourses.join(", ")
    }`;
  } else if (!isTwoOrMoreCompleted) {
    message =
      `システムプログラム系科目の単位修得が${completedCourses.length}科目で不足しています（要件: ${requiredCompletedCount}科目以上）`;
  }
...
export const checkJabeeSystemProgrammingFullRequirement: RequirementCheckFn = (
  ...
  return {
    name:
      "JABEE専門科目群：4. システムプログラム系科目において、5科目以上の単位修得",
    satisfied,
    message: satisfied
...
export const checkJabeeInformationCommunicationRequirement: RequirementCheckFn =
  (
    ...
    const completedOrPlannedCourses = informationCommunicationCourses.filter(
      (item) => {
    ...
    if (satisfied) {
      message =
        `JABEE専門科目群：情報通信・信号処理系科目の要件を満たしています (${completedCount}科目 / 必要${requiredCourseCount}科目)`;
    } else {
      message =
        `JABEE専門科目群：情報通信・信号処理系科目の要件を満たしていません (${completedCount}科目 / 必要${requiredCourseCount}科目)`;
    }

    // 未完了の科目のリスト
    const incompleteCourses = informationCommunicationCourses.filter(
      (course) =>
        !completedOrPlannedCourses.some((c) => c.科目番号 === course.科目番号),
    );

    return {
      name:
        "JABEE専門科目群：5. 情報通信・信号処理系科目について、4科目以上の単位修得",
      satisfied,
      message,
      details: {
        total: requiredCourseCount,
...
export const checkJabeeComputerApplicationRequirement: RequirementCheckFn = (
  ...
  return {
    name:
      "JABEE専門科目群：6. コンピュータ応用系科目について、3科目以上の単位修得",
    satisfied,
    message: satisfied
...
export const checkJabeeMathematicalScienceRequirement: RequirementCheckFn = (
  ...
  return {
    name:
      "JABEE専門科目群：7. 数理科学系科目について、情報数学Ⅰと情報数学Ⅱから1科目以上の単位修得",
    satisfied,
    message: satisfied
...
export const checkJabeeMathematicalScienceFullRequirement: RequirementCheckFn =
  (
    ...
    return {
      name:
        "JABEE専門科目群：8. 数理科学系科目について、選択必修科目を含む4科目以上の単位修得",
      satisfied,
      message: satisfied
        ? `数理科学系科目を${completedCourses.length}科目修得済み（要件: ${requiredCourseCount}科目以上）`
...
export const checkJabeeExperimentPracticeRequirement: RequirementCheckFn = (
  ...
  const allCoursesExist = EXPERIMENT_PRACTICE_REQUIRED_COURSES.every((
    courseName,
  ) => experimentPracticeCourses.some((item) => item.授業科目 === courseName));

  // シラバスに含まれていない科目リスト
  const missingCourses = EXPERIMENT_PRACTICE_REQUIRED_COURSES.filter((
    courseName,
  ) => !experimentPracticeCourses.some((item) => item.授業科目 === courseName));
  ...
  let message = "";
  if (!allCoursesExist) {
    message = `シラバスに含まれていない実験・実習系必修科目があります: ${
      missingCourses.join(", ")
    }`;
  } else if (!satisfied) {
    message =
      `実験・実習系必修科目の単位修得が不足しています（現在: ${completedCourses.length}科目、必要: ${totalRequiredCourses}科目）`;
  } else {
    message =
      `実験・実習系必修科目をすべて修得済み（${completedCourses.length}科目）`;
  }

  return {
    name:
      "JABEE専門科目群：9. 実験・実習系科目について、すべての必修科目の単位修得",
    satisfied,
    message,
    details: {
